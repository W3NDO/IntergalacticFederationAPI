...
RSpec.describe Resource, type: :model do
  let(:resource_1) {Resource.new(name: 1, weight: 10)}
  let(:resource_2) {Resource.new(name: 2, weight: 10)}
  let(:resource_3) {Resource.new(name: 3, weight: 10)}
  let(:invalid_resource) {Resource.new(name: 2, weight: 0)}
  
  it "ensures that you can only create 3 types of resources" do
    expect{ Resource.new(name: 4, weight: 0) }.to raise_error(ArgumentError)
